...
name: graphify
description: >
  Build a visual knowledge graph from any input — documents, code, papers, or images.
  Use when user asks to map relationships between concepts, visualise a domain's structure,
  or generate an interactive HTML graph for a set of files or notes.
trigger: /graphify
---
...
```bash
uv tool install 'graphifyy[mcp]'   # includes MCP server support
graphify install --platform hermes  # installs skill → ~/.hermes/skills/graphify/SKILL.md
```

### Indexed Graphs

| Repo | Graph path | Stats |
|------|-----------|-------|
| hermes-agent | `~/.hermes/hermes-agent/graphify-out/graph.json` | 31,767 nodes · 58,893 edges · 2,042 communities |

Graph is code-only (AST via tree-sitter, **no API cost**). Docs/PDFs/images require LLM extraction.

### Update graph after code changes (no LLM needed)

```bash
cd ~/.hermes/hermes-agent
graphify update . --no-viz    # fast, AST-only, ~1 min
```
...
mcp_servers:
  graphify:
    ...
    - -m
    - graphify.serve
    - ~/.hermes/hermes-agent/graphify-out/graph.json
    transport: stdio
    timeout: 60
    connect_timeout: 30
```

After Hermes restart, tools appear as `mcp_graphify_*`:

| MCP Tool | Purpose |
|---|---|
| `mcp_graphify_query_graph` | BFS/DFS traversal — broad context or trace a path |
| `mcp_graphify_get_node` | Full details for a specific node by label |
| `mcp_graphify_get_neighbors` | All direct neighbors of a node |
| `mcp_graphify_get_community` | All nodes in a community cluster |
| `mcp_graphify_god_nodes` | Most-connected nodes (architecture hubs) |
| `mcp_graphify_graph_stats` | Node/edge/community summary |
| `mcp_graphify_shortest_path` | Shortest path between two concepts |

### Test MCP connection

```bash
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"1"}}}' \
  | uv tool run --from 'graphifyy[mcp]' python -m graphify.serve \
    ~/.hermes/hermes-agent/graphify-out/graph.json 2>/dev/null | head -1
# → {"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2024-11-05",...}}
```

### Pitfalls

- **Graph too large for HTML**: hermes-agent has 31,767 nodes — HTML viz is disabled (limit 5,000). Always use `--no-viz`.
- **`uv tool run --from graphifyy[mcp]`** — the bracket syntax matters; use `'graphifyy[mcp]'` in shell (single quotes).
- **Graph path is hardcoded** in MCP config — if you index a new repo, add a second graphify MCP entry (or update the path).
- **Code is local, docs need LLM** — `graphify update .` is free (AST only). Full `/graphify .` for docs/images/PDFs needs ANTHROPIC_API_KEY.
- **Graph staleness** — after `git pull` on hermes-agent, run `graphify update .` to stay current. Check `GRAPH_REPORT.md` header for the indexed commit hash.
