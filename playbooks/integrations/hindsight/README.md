...
|---|---|---|---|
| `[org]-global` | Everyone | Iris | Company identity, product facts |
| `[org]-internal` | All agents | Iris | Ops processes, tooling conventions |
| `[org]-sales` | Leo, Maya, Rex, Iris, Humans | Leo, Iris | Opportunities, opportunity context, what was promised, partner interactions |
| `[org]-agent-{name}` | That agent only | That agent | Role-specific patterns, learnings |
| `[org]-human-{name}` | That human only | Iris + that human | Personal context, preferences |
...
Agents read from Hindsight when they need interaction history before acting — e.g. Leo checks `[org]-sales` before drafting a follow-up to understand what was last said.
...
| Company policies | [org]-knowledge-base |
