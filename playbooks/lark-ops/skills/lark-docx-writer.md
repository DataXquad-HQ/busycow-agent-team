...
Lark Docx write access depends on which **Feishu app** is a collaborator on the document. The default BusyCow app (`~/.hermes/.env`) will return **HTTP 403** on write calls if the document was created under the [your product] (or another) tenant/app.

**Rule:** Match credentials to the document's owning app/profile:
- [your product] docs → use `~/.hermes/profiles/aquaoptima/.env` credentials
- BusyCow docs → use `~/.hermes/.env` credentials
- If you get 403 on write but read succeeds, switch to the correct profile's `.env`

```python
# [your product] profile example
...
**⚠️ `file_name` is limited to 27 characters** — passing a longer name returns HTTP 422 validation error. Shorten aggressively (e.g., `"[your product] Apr'26 Update v2"` not `"[your product] Investor Update — Apr '26 (Updated)"`).
...
**Credential note:** `mcp_lark_docx_builtin_import` uses the default BusyCow app credentials. If writing to an [your product]-owned doc, you still need the [your product] profile credentials for block-level edits — but `builtin_import` creates a new doc under the BusyCow app's accessible space.
...
```python
# Preferred fallback:
session_search(query="[your product] investor update document")
...
After calling `mcp_lark_docx_builtin_import`, the created Doc is NOT accessible to Hunter by default.
...
### Step 4: Grant access to Hunter

Wiki nodes created by the App are not visible to Hunter by default. Always call:
