...
- `open.feishu.cn` — China / Feishu region (e.g. [Product] / MTR-AVIS bases)
...
**Known open_ids:**
- the owner: `{{USER_OPEN_ID}}`
- the admin: `{{USER_OPEN_ID}}`
...
- **App Token**: `{{SALES_OPS_APP_TOKEN}}`
- **URL**: https://cjpg0xp67g6h.jp.larksuite.com/base/{{SALES_OPS_APP_TOKEN}}
...
**Emoji prefix pattern** (the owner prefers this for grouping):
...
- `{{USER_OPEN_ID}}` — the owner (session user / owner)
...
- **App Token**: `{{SALES_OPS_APP_TOKEN}}`
- **URL**: https://bytedance.larkoffice.com/base/{{SALES_OPS_APP_TOKEN}}
- Owner: Hermes-BusyCow bot (`cli_a97bd21895f89e18`)
- the owner + the admin have `full_access`
...
**Entity logic**: [Product]/[Product] → TW - ATA Limited 應科聯; BusyCow/[Product]/Others → SG - BusyCow Pte. Ltd. (default for overseas income = SG, Taiwan income = TW unless instructed otherwise)
...
- **App Token**: `TVhhbod2haFHdtsEhmmjy12lpAh`
- **Tasks Table**: `{{TABLE_ID}}`
- **Projects Table**: `{{TABLE_ID}}`
- **URL**: https://cjpg0xp67g6h.jp.larksuite.com/base/TVhhbod2haFHdtsEhmmjy12lpAh
- Covers all 4 business lines: BusyCow, [Product], [Product], [Product]
- the owner + the admin shared; Hermes maintains it (weekly rhythm, no manual maintenance needed)
...
- **Admins**: the owner (`ou_9ba57313`) — full_access
...
`Product Line` select options: 🐄 BusyCow, 💧 [Product], 🏢 BusyCow, ⚙️ System
